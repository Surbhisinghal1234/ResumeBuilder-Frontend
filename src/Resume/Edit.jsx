...
function Edit() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://resumebuilder-backend-1.onrender.com/api/getById/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = id
        ? await axios.put(`https://resumebuilder-backend-1.onrender.com/api/${id}`, formData, {
            headers: { "Content-Type": "multipart/form-data" },
          })
        : await axios.post("https://resumebuilder-backend-1.onrender.com/api/send", formData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
